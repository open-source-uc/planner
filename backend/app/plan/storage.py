...
_plan_model_fields = list(DbPlan.model_fields.keys())
...
class PlanView(BaseModel):
    ...
    def from_db(db: DbPlan) -> "PlanView":
        return PlanView(
            ...
            validatable_plan=ValidatablePlan.model_validate_json(db.validatable_plan),
        )
...
async def store_plan(plan_name: str, user: UserKey, plan: ValidatablePlan) -> PlanView:
    ...
    data = {
        ...
        "user_rut": user.rut,
        "is_favorite": False,
        "validatable_plan": plan.model_dump_json(),
    }
...
async def modify_validatable_plan(
    ...
    updated_plan = await DbPlan.prisma().update(
        where={"id": plan_id},
        data={"validatable_plan": new_plan.model_dump_json()},
    )
