...
class LinearExpr:
    ...
    def __add__(self, expr: "LinearExpr") -> "LinearExpr":
        return Sum(self, expr)

    def __radd__(self, cst: "LinearExpr") -> "LinearExpr":
        return Sum(self, cst)

    def __sub__(self, expr: "LinearExpr") -> "LinearExpr":
        return Sum(self, -expr)

    def __rsub__(self, cst: "LinearExpr") -> "LinearExpr":
        return Sum(-self, cst)

    def __mul__(self, cst: float | int):
        return ProductCst(self, cst)

    def __rmul__(self, cst: float | int):
        return ProductCst(self, cst)

    def __div__(self, cst: float | int):
        return ProductCst(self, 1.0 / cst)

    def __truediv__(self, cst: float | int):
        return ProductCst(self, 1.0 / cst)

    def __neg__(self):
        return ProductCst(self, -1)

    def __eq__(self, arg: "LinearExpr") -> "LinearConstraint":
        if isinstance(arg, numbers.Number):
            return LinearConstraint(self, arg, arg)
        else:
            return LinearConstraint(self - arg, 0.0, 0.0)

    def __ge__(self, arg: "LinearExpr | float") -> "LinearConstraint":
        if isinstance(arg, numbers.Number):
            return LinearConstraint(self, arg, inf)
        else:
            return LinearConstraint(self - arg, 0.0, inf)

    def __le__(self, arg: "LinearExpr | float") -> "LinearConstraint":
        if isinstance(arg, numbers.Number):
...
class LinearConstraint:
    ...
    def __init__(self, expr: LinearExpr, lb: float, ub: float) -> None:
        self.__expr = expr
        self.__lb = lb
        self.__ub = ub
