...
UNKNOWN: int = cp_model_pb2.UNKNOWN
MODEL_INVALID: int = cp_model_pb2.MODEL_INVALID
FEASIBLE: int = cp_model_pb2.FEASIBLE
INFEASIBLE: int = cp_model_pb2.INFEASIBLE
OPTIMAL: int = cp_model_pb2.OPTIMAL
...
class LinearExpr:
    ...
    * For large arrays, using the LinearExpr class is faster that using the python
    ...
    def IsEmptyOrAllNull(cls, coefficients):
        return all(cmh.is_zero(c) for c in coefficients)
    @classmethod
    ...
    def __add__(self, arg: IntOrLinearExpr) -> IntOrLinearExpr:
        if cmh.is_zero(arg):
            return self
        return _Sum(self, arg)
    def __radd__(self, arg: IntOrLinearExpr) -> IntOrLinearExpr:
        if cmh.is_zero(arg):
            return self
        return _Sum(self, arg)
    def __sub__(self, arg: IntOrLinearExpr) -> IntOrLinearExpr:
        if cmh.is_zero(arg):
            return self
        return _Sum(self, -arg)
    def __rsub__(self, arg: IntOrLinearExpr) -> IntOrLinearExpr:
        return _Sum(-self, arg)
    def __mul__(self, arg: IntOrLinearExpr) -> IntOrLinearExpr:
    ...
    def __rmul__(self, arg: IntOrLinearExpr) -> IntOrLinearExpr:
        arg = cmh.assert_is_a_number(arg)
        if cmh.is_one(arg):
            return self
        elif cmh.is_zero(arg):
...
class IntVar(LinearExpr):
    ...
    def __init__(self, model, domain, name) -> None:
    ...
    def __repr__(self) -> str:
        return f"{self.__var.name}({DisplayBounds(self.__var.domain)})"
    def Name(self):
        return self.__var.name
    def Not(self):
...
class BoundedLinearExpression:
    ...
    def __init__(self, expr, bounds) -> None:
        self.__expr = expr
...
class IntervalVar:
    ...
    def __init__(self, model, start, size, end, is_present_index, name) -> None:
        self.__model = model
    ...
    def StartExpr(self):
        return LinearExpr.RebuildFromLinearExpressionProto(
            self.__model,
            self.__ct.interval.start,
        )
    def SizeExpr(self):
        return LinearExpr.RebuildFromLinearExpressionProto(
            self.__model,
            self.__ct.interval.size,
        )
    def EndExpr(self):
        return LinearExpr.RebuildFromLinearExpressionProto(
            self.__model,
            self.__ct.interval.end,
        )
...
class CpModel:
    ...
    * ```Add``` create new constraints and add them to the model.
    ...
    def NewIntVarFromDomain(self, domain, name) -> IntVar:
    ...
    def Add(self, ct):
        ...
        if isinstance(ct, BoundedLinearExpression):
            return self.AddLinearExpressionInDomain(
                ct.Expression(),
                Domain.FromFlatIntervals(ct.Bounds()),
            )
    ...
    def AddImplication(self, a: IntVar, b: IntVar) -> Constraint:
    ...
    def AddBoolOr(self, *literals):
        ...
        return ct
    def AddAtLeastOne(self, *literals: IntVar | Iterable[IntVar]) -> Constraint:
        """Same as `AddBoolOr`: `Sum(literals) >= 1`."""
        return self.AddBoolOr(*literals)
    def AddAtMostOne(self, *literals: IntVar | Iterable[IntVar]) -> Constraint:
    ...
    def AddMinEquality(
        self,
        target: IntOrLinearExpr,
        exprs: Iterable[IntOrLinearExpr],
    ) -> Constraint:
        ...
        return ct
    def AddMaxEquality(self, target: IntOrLinearExpr, exprs: Iterable[IntOrLinearExpr]):
    ...
    def Minimize(self, obj: IntOrLinearExpr) -> None:
        """Sets the objective of the model to minimize(obj)."""
        self._SetObjective(obj, minimize=True)
    def Maximize(self, obj: IntOrLinearExpr) -> None:
...
def EvaluateBooleanExpression(literal, solution):
    ...
    if cmh.is_integral(literal):
        return bool(literal)
    elif isinstance(literal, IntVar | _NotBooleanVariable):
        index = literal.Index()
        if index >= 0:
...
class CpSolver:
    ...
    def Value(self, expression: LinearExpr) -> int:
    ...
    def BooleanValue(self, literal: IntVar) -> bool:
    ...
    * `UserTime(self)`
    ...
    def BooleanValue(self, lit):
        ...
        if cmh.is_integral(lit):
            return bool(lit)
        elif isinstance(lit, IntVar | _NotBooleanVariable):
            index = lit.Index()
...
IntOrLinearExpr = int | LinearExpr
