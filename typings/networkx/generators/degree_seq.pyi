...
chaini = ...
@py_random_state(2)
def configuration_model(deg_sequence, create_using=..., seed=...):
...
@py_random_state(3)
def directed_configuration_model(in_degree_sequence, out_degree_sequence, create_using=..., seed=...):
...
@py_random_state(1)
def expected_degree_graph(w, seed=..., selfloops=...):
...
def havel_hakimi_graph(deg_sequence, create_using=...):
...
def directed_havel_hakimi_graph(in_deg_sequence, out_deg_sequence, create_using=...):
...
def degree_sequence_tree(deg_sequence, create_using=...):
...
@py_random_state(1)
def random_degree_sequence_graph(sequence, seed=..., tries=...): # -> Graph:
