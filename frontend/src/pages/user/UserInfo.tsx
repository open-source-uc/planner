      ...
      <div className="flex flex-col mb-4 h-full sentry-mask">
          <div className="h-1/2 m-3">
              ...
              <ul className="m-3 space-y-2">
                  <li>Nombre: {authState?.student?.full_name}</li>
                  {/* TODO: Fix this when we have time */}
                  {/* <li>Ingreso: {String(authState?.student?.admission.slice(0, -1)) + '-' + String(authState?.student?.admission.slice(-1))}</li> */}
          ...
          <div className="h-1/2 m-3">
              ...
              <ul className="m-3 space-y-2">
                  <li>Título: {authState?.student?.reported_title ?? 'No Inscrito'} </li>
                  <li>Major: {authState?.student?.reported_major ?? 'No declarado'} </li>
                  <li>Minor: {authState?.student?.reported_minor ?? 'No declarado'} </li>
              </ul>
          </div>
      </div>
    </>
